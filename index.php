...
$req_total = $dbh->prepare("SELECT COUNT(*) as nb FROM listings");
$req_total->execute();
$data_total = $req_total->fetch();
$total = $data_total['nb'];
$nombre_pages = ceil($total / $nombre_par_page);
 
$requete_logements = $dbh->prepare("SELECT * FROM listings ORDER BY $ordre LIMIT $debut, $nombre_par_page");
$requete_logements->execute();
$resultat = $requete_logements->fetchAll();
?>
...
<td>
<img src="<?php echo $ligne['host_thumbnail_url']; ?>" width="30" style="border-radius: 50%; vertical-align: middle;">
<?php echo $ligne['host_name']; ?>
</td>
</tr>
<?php
        }
